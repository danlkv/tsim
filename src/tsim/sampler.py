...
class _CompiledSamplerBase:
    ...
    def _sample_batches(
        ...
        *,
        ...
        for _batch_i in range(ceil(shots / batch_size)):
        ...
        # Concat on device first, then a single d2h into one numpy buffer. The
        # prior np.concatenate(batches) triggered per-batch __array__ d2h + a
        # second host-side memcpy into the concat output — for big bool tensors
        # the host concat alone was ~1 s on star7 500k. Device concat folds
        # everything into one PCIe transfer.
        ...
        # Compute wall ends here; d2h wall starts. perf_counter readings let the
        # bench separate the two without nsys.
